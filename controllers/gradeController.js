...
const remove = async (req, res) => {
  ...
  try {
    ...
    if (!student) {
      res.status(404).send('Documento não encontrado ' + error);
    } else {
      res.status(200).send();
...
const removeAll = async (req, res) => {
  try {
    const student = await studentModel.remove({});
    res.status(200).send(student.deletedCount + ' documentos removidos');
